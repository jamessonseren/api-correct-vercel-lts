...
export class CreateAppUserItemByEmployerUsecase {
  ...
  async execute(data: InputCreateAppUserItemByEmployerDTO){
    ...
    if(!userInfo.business_info_uuid || userInfo.business_info_uuid?.uuid !== data.business_info_uuid) throw new CustomError("Unauthorized access", 403)
    ...
    return {
      ...
      business_info_uuid: itemDetails.business_info_uuid,
