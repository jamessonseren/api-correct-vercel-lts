...
export class GetUserInfoByUserUsecase{
    ...
    async execute(input: InputFindUserByUserDTO): Promise<OutputFindUserByUserDTO>{
        if(!input.document) throw new CustomError("Document is required", 400)

        const userInfo = await this.appUsersRepository.findByDocumentUserInfo(input.document)
